...
@Component({
  ...
  styleUrl: './pag-formulario.component.css'
})
export class PagFormularioComponent implements OnInit {
  ...
  private user: string = "rodrigo";
  private pass: string = "01";
  ...
  generateToken(): void {
    ...
    if ((this.bearerToken.length == 0) || (currentTime > this.lastTime + minutesTst)) {
      ...
      this.http.post<any>('http://localhost:8081/rest/api/oauth2/v1/token?grant_type=password&password=' + passDecode + '&username=' + this.user, { /* httpOptions */ }).subscribe({
        next: (v) => {
  ...
  validCNPJInput(): void {
    ...
    this.http.get<any>('http://localhost:8081/rest/zWsTransport/get_id?id=' + cnpjValue, httpOptions).subscribe({
      next: (v) => {
  ...
  clickedOnConfirm(): void {
    ...
    this.http.post<any>('http://localhost:8081/rest/zWsTransport/new', httpBody, httpOptions).subscribe({
      next: (v) => {
        this.clickedOnClear();
      },
      error: (e) => {
